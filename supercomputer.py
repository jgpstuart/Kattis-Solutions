...
class SegmentTree(object):
    ...
    def _update(self, vertex: int, left: int, right: int, index: int,
                value: T):
        if left == right:
            self.tree[vertex] = 0 if self.tree[vertex] == 1 else 1
        else:
            middle = (left + right) // 2
            if index <= middle:
...
bits, queries = [int(x) for x in input().split()]
st = SegmentTree([0]*bits, add)


for i in range(queries):
    instruction = input().split() #take in instruction
    if instruction[0] == "F":
        st.update(int(instruction[1])-1, 0)
    if instruction[0] == "C":
        num_1 = st.range_query(int(instruction[1])-1,int(instruction[2])-1)
        print(num_1)
